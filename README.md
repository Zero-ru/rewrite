...
В данный момент  для скачивания доступна только бесплатная версия родукта. PRO версия находится в разработке.
...
Главное меню Framework. В меню доступно 45 функций для поиска. От простого до сложного. Сейчас пополняется только одно меню.
...
Как только все API введены, нужно установить зависимости:

     sudo pip3 install -r requirements.txt
    
     В сложных модулях, будет предоставлен выбор дополнительной установки модулей.
---

### Баннер авторизации.

Запуск осуществляется командой: python3 osintsan.py

Пароль и логин хранится в нашем паблике в telegram.

Инструменты запускаются в двух режимах:

    Для запуска программы без root [ функции выделены зеленым ], ввести команду :  python3 osintsan.py 
      
    Для запуска программы с root [ функции выделены красным ], ввести команду : sudo python3 osintsan.py
 
Official public https://t.me/osint_san_framework

![alt tag](https://github.com/Bafomet666/screen/blob/main/login.png)

Меню инструментов будет доступно после авторизации

---

### Функция [ 01 ]
Проверка IP-адреса в Shodan and Censys на утечки.
Для получения информации необходимо ввести IP-адрес

![alt tag](https://github.com/Bafomet666/screen/blob/main/0130.png)

### Функция [ 02 ] РЕДАКТИРУЕТСЯ
...
### Функция [ 04 ]
Позволяет получить детальную карту DNS ping,имеющую высокое разрешение.

![alt tag](https://github.com/Bafomet666/screen/blob/main/044.png)

### Функция [ 05 ]
Позволяет извлечь геолокацию из фото.
### Функция [ 06 ]
Осуществляет поиск по фото.
...
### Функция [ 08 ]
 ...
 Mac address info, с вероятностью 40% может показать и геолокацию.
 
![alt tag](https://github.com/Bafomet666/screen/blob/main/88854.png)

### Функция [ 09 ]
Позволяет вычислить местоположение по IP
