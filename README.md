...
Инструменты запускаются в двух режимах:
    ...
    Если Вам нужны функции без root [ помечены зеленым цветом ], запускайте командой в терминале:  python3 osintsan.py 
      
    Если Вам нужны функции root [ помечены красным цветом ],запускайте командой в терминале: sudo python3 osintsan.py
...
Находите адреса mail с сайтов. Используется API censys.
